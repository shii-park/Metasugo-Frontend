...
import { getActiveSocket } from '@/lib/game/wsClient'; // ★ 追加
...
export default function Branch({ onClose }: BranchProps) {
  ...
  // ★ 追加: サーバからのクイズ要求を読む
  const quizReq = useGameStore((s) => s.quizReq)
  const clearQuizReq = useGameStore((s) => s.clearQuizReq)
  ...
  const copy = PAGE_COPY[currentPage] ?? { title: `【クイズマス：${currentPage}ページ目】` }

  // ★ 表示テキストはサーバのクイズがあればそれを使う（既存UIはフォールバック）
  const question = quizReq?.quizData.question ?? 'よしくんの苗字は？'
  const optA = quizReq?.quizData.options?.[0] ?? '吉元'
  const optB = quizReq?.quizData.options?.[1] ?? '吉本'

  // ★ クイズ終了時の片付け（閉じる時に store をクリア）
  ...
  const go = (choice: Choice) => {
    ...
    // ★ WebSocket に SUBMIT_QUIZ を送る（A=0, B=1）
    const index = choice === 'a' ? 0 : 1
    getActiveSocket()?.sendSubmitQuiz(index)

    // 以降は既存のUIフロー（結果表示→閉じる）
    const isCorrect = choice === 'a' // UI上の演出は従来どおり維持（正誤はサーバ側で判定されてPLAYER_MOVED/MONEY_CHANGEDが飛んできます）
    const finalMessage = isCorrect ? '正解' : '不正解'
