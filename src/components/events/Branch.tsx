...
export default function Branch({ onClose }: BranchProps) {
 ...
 const stopCamera = () => {
  ...
  const handleCodeDetection = (data: string) => {
    ...
    const answerList = correctAnswwers ? correctAnswwers.join('、'):'設定なし';
 ...
 return (
  <div className='absolute z-50 inset-0 cursor-pointer' onClick={isTitleOnly && !isCameraActive ? handleAdvance : undefined}>
        {isCameraActive && (
            <div className='flex justify-center items-center w-full h-full absolute z-50 inset-0 bg-black/50'>
                <div className='rounded-[6] border-2 border-black/90 bg-white/90 backdrop-blur-sm shadow-lg p-5 w-5/8 relative'>
                  <div className='relative overflow-hidden rounded-md aspect-video'>
                    <video ref={videoRef} autoPlay playsInline className='w-full' />
                    <canvas ref={canvasRef} style={{ display: 'none' }} /> 
                  </div>
                    
                    <p className="text-center text-[#5B7BA6] font-bold text-lg p-2 mt-2 border-t border-gray-200">
                        正解と思うQRコードを読み取ってね
                    </p>
                    <p className="text-center text-[#5B7BA6] font-bold text-lg p-2 mt-2 border-t border-gray-200">
                        問題:{copy.message}
    ...
    {!isCameraActive && (
             <div className='absolute inset-0'> 
                {isTitleOnly ? (
                    ...
                    <div className='flex justify-center items-center w-full h-full absolute z-50 inset-0 bg-brack/50'>
                        <div className="rounded-[6] border-2 border-black/90 bg-white/90 backdrop-blur-sm shadow-lg p-5 w-5/8">
                            ...
                            <div className="rounded-xl border-2 bg-[#ccb173] backdrop-blur-sm shadow-lg md:p-5 text-center">
                                <button
                                    ...
                                    disabled={submitting}
                                >
                                    <img src="/QR_example.svg" alt="QRコード" className=" pr-[8%]" />
                                    QRコードを読み取る
                                </button>
                            </div>
                ...
                {/* 吹き出し三角の表示 */}
                <div className="relative absolute z-50 left-[5%] right-[5%] bottom-[6%]">
